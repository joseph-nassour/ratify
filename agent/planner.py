...
class DeterministicPlanner:
    ...
    def tool_plan(self) -> List[ToolStep]:
        return [
            ...
            ToolStep("compose_draft",
                     "Draw a preview locally — this process has no vendor credential",
                     gated=False),
            ToolStep("docgen.GenerateDocumentBase64",
                     "Delegated: the parent renders the signable document from the "
                     "ratified terms", gated=False),
            ToolStep("mcp.merge_pdf", "Delegated: merge the standard terms & conditions pages",
                     gated=False),
            ToolStep("mcp.flatten_pdf", "Delegated: flatten the merged document", gated=False),
            ToolStep("mcp.extract_text",
                     "Delegated: read the rendered PDF back and verify the terms it "
                     "actually contains", gated=False, material=False),
